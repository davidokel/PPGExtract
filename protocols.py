...
def extraction_protocol_v3(nicp_dataset, icp_dataset, fs, window_size, visualise = False, debug = False, z_score_threshold = 3, z_score_detection = True, derivative=[]):
    # Check if window_size is 0 or negative
    if window_size <= 0:
        raise ValueError("window_size must be greater than 0")
    ...
    for column in nicp_dataset.columns:
        ...
        for window in range(num_windows):
            ...
            # Extracting the pulses from the current window #
            #################################################
            peak_points, peaks, troughs = get_pulses(list(data_window), fs=fs, visualise=True, debug=debug, z_score_threshold=z_score_threshold, z_score_detection=z_score_detection)
            features = get_features_v2(peak_points, fs, visualise=visualise, debug=debug)
            patient_features[window] = features
            ...
            if derivative:
                ...
                for i in derivative:
                    ...
                    # Extracting the pulses from the current window #
                    #################################################
                    peak_points, peaks, troughs = get_pulses(list(data_window), fs=fs, visualise=True, debug=debug, z_score_threshold=z_score_threshold, z_score_detection=z_score_detection, derivative=i)
                    features = get_features_v2(peak_points, fs, visualise=visualise, debug=debug)
                    # Change the name of each value in the dictionary to include the derivative
                    features = {key + "_deriv_" + str(i): value for key, value in features.items()}
                    deriv_dictionary.update(features)
